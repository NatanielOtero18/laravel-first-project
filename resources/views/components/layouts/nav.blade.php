    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-16 lg:px-20">
        <div class="relative flex items-center justify-between h-16">
            <div class="flex items-center justify-center flex-1 sm:items-stretch sm:justify-start">
                ...
                <div style="display: flex; align-items: flex-start; jutify-content: center">
            ...
            @guest
                <div style="display: flex; align-items: flex-end; jutify-content: flex-end">
                    ...
                    <a href="{{ route('login') }}"
                        class="px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600 {{ request()->routeIs('login') ? 'text-sky-600' : 'text-slate-400' }}">
                        Login
                    </a>
                </div>
            @endguest
            @auth
                <a href="{{ route('login') }}"
                    class="px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600 ">
                    {{ Auth::user()->name }}
                </a>
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <a href="#" class="px-3 py-2 text-sm font-medium rounded-md hover:text-sky-600"
